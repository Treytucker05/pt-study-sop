...
function safeDateLabel(raw: string | Date | null | undefined) {
  if (!raw) return "Unknown";
  const parsed = raw instanceof Date ? raw : new Date(raw);
  return Number.isNaN(parsed.getTime()) ? String(raw) : format(parsed, "MMM d, yyyy");
}
...
function toList(value: unknown): string[] {
  ...
  try {
    ...
    if (Array.isArray(parsed))
      return parsed.map(stringifyEntry).filter(Boolean);
  } catch {
      ...
      <CardHeader className="border-b border-primary/15 px-4 pb-3 pt-3">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1">
            <CardTitle className="font-arcade text-sm uppercase tracking-[0.18em] text-primary">
              {title}
            </CardTitle>
            <CardDescription className="font-mono text-sm leading-6 text-foreground/68">
              {description}
            </CardDescription>
          </div>
        </div>
...
export function ContractBrainHome({
  workspace,
}: {
  ...
  const lowYieldSessions = useMemo(
    () => buildLowYieldSessions(sortedSessions).slice(0, 5),
    [sortedSessions],
  );
  const totalIssuePatterns = (metrics?.issuesLog || []).reduce(
    ...
    >
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <Card className={CONTROL_DECK}>
          ...
          <CardHeader className="relative z-10 border-b border-primary/15">
          ...
          >
            <div className="rounded-[1rem] border border-primary/15 bg-black/30 p-3">
              ...
              <div className="mt-1 font-mono text-sm leading-6 text-foreground/68">
                {sortedSessions[0]
                  ? `Latest ${safeDateLabel(sortedSessions[0].date)}`
            ...
            <div className="rounded-[1rem] border border-primary/15 bg-black/30 p-3">
              ...
              <div className="mt-1 font-mono text-sm leading-6 text-foreground/68">
                Workflow, source-lock, and interruption signals
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <SectionCard
          ...
          description="One row per stored study session. Raw WRAP fields only."
          testId="brain-session-evidence"
        >
          {sessionsLoading ? (
            ...
            <div className="overflow-x-auto border border-primary/15 bg-black/30">
              <table className="min-w-full border-collapse font-terminal text-xs">
                <thead className="bg-black/55 text-ui-xs uppercase tracking-[0.18em] text-primary/80">
                  <tr>
                    <th className="border-b border-primary/15 px-3 py-2 text-left">
                      Date
                    </th>
                    <th className="border-b border-primary/15 px-3 py-2 text-left">
                      Course
                ...
                <tbody>
                  {sortedSessions.map((session) => (
                    <tr
                      key={session.id}
                      className="align-top text-white odd:bg-black/10"
                    >
                      <td className="border-b border-primary/10 px-3 py-2 whitespace-nowrap">
                        {safeDateLabel(session.date)}
                      </td>
                      <td className="border-b border-primary/10 px-3 py-2">
                        {session.topic}
                      </td>
                      <td className="border-b border-primary/10 px-3 py-2 text-muted-foreground">
                      ...
                      <td className="border-b border-primary/10 px-3 py-2 text-muted-foreground">
                        {previewList(session.confusions)}
                      </td>
                      <td className="border-b border-primary/10 px-3 py-2 text-muted-foreground">
                        {previewList(session.weakAnchors)}
                      </td>
                      <td className="border-b border-primary/10 px-3 py-2 text-muted-foreground">
                        {previewList(session.concepts)}
                      </td>
                      <td className="border-b border-primary/10 px-3 py-2 whitespace-nowrap">
                        {session.cards || 0}
                      </td>
                      <td className="border-b border-primary/10 px-3 py-2 text-muted-foreground">
                        {previewList(session.issues)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="border border-primary/15 bg-black/30 p-4 font-terminal text-sm text-muted-foreground">
              No stored sessions yet. Brain will populate once WRAP outputs are
              logged.
            </div>
          )}
        </SectionCard>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          <SectionCard
            ...
            description="Rollups derived from the same WRAP fields shown above."
            testId="brain-derived-metrics"
          >
            <div className="grid gap-3 md:grid-cols-2">
              <div
                className="border border-primary/15 bg-black/30 p-3"
                data-testid="brain-course-rollup"
              >
                <div className="font-arcade text-ui-xs text-primary">
                  SESSIONS / COURSE
                </div>
                <div className="mt-3 space-y-2">
                  {(metrics?.sessionsPerCourse || []).length ? (
                    metrics?.sessionsPerCourse.map((item) => (
                      <div
                        key={item.course}
                        className="border border-primary/10 bg-black/35 px-3 py-2"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="font-terminal text-xs text-white">
                            {item.course}
                          </div>
                          <Badge
                            variant="outline"
                            className="rounded-none border-primary/25 text-ui-2xs text-primary"
                          >
                            {item.count}
                          </Badge>
                        </div>
                        <div className="mt-1 font-terminal text-ui-xs text-muted-foreground">
                          {item.minutes} minute(s) logged
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="font-terminal text-xs text-muted-foreground">
                      No course rollup yet.
                    </div>
                  )}
                </div>
              </div>
              <div
                className="border border-primary/15 bg-black/30 p-3"
                data-testid="brain-mode-rollup"
              >
                <div className="font-arcade text-ui-xs text-primary">
                  LOGGED MODE VALUES
                </div>
                <div className="mt-1 font-terminal text-ui-xs text-muted-foreground">
                  Frequency of the raw <code>session.mode</code> field as stored
                  today.
                </div>
                <div className="mt-3 space-y-2">
                  {(metrics?.modeDistribution || []).length ? (
                    metrics?.modeDistribution.map((item) => (
                      <div
                        key={item.mode}
                        className="border border-primary/10 bg-black/35 px-3 py-2"
                      >
              ...
              <div
                className="border border-primary/15 bg-black/30 p-3"
                data-testid="brain-concept-rollup"
              >
                <div className="font-arcade text-ui-xs text-primary">
                  CONCEPT REVISITS
                </div>
                <div className="mt-3 space-y-2">
                  {(metrics?.conceptFrequency || []).length ? (
                    metrics?.conceptFrequency.map((item) => (
                      <div
                        key={item.concept}
                        className="flex items-center justify-between gap-3 border border-primary/10 bg-black/35 px-3 py-2"
                      >
                        <div className="font-terminal text-xs text-white">
                          {item.concept}
                        </div>
              ...
              <div
                className="border border-primary/15 bg-black/30 p-3"
                data-testid="brain-low-yield"
              >
                <div className="font-arcade text-ui-xs text-primary">
                  HIGH-EFFORT / LOW-YIELD
                </div>
                <div className="mt-3 space-y-2">
                  {lowYieldSessions.length ? (
                    lowYieldSessions.map((session) => (
                      <div
                        key={session.id}
                        className="border border-primary/10 bg-black/35 px-3 py-2"
                      >
                        <div className="font-terminal text-xs text-white">
                          {session.topic} • {safeDateLabel(session.date)}
                        </div>
                        <div className="mt-1 font-terminal text-ui-xs text-muted-foreground">
                          {session.minutes} minute(s) •{" "}
                          {previewList(session.issues, "No issue text")}
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="font-terminal text-xs text-muted-foreground">
                      No low-yield sessions detected from current WRAP evidence.
                    </div>
                  )}
                </div>
              </div>
          ...
          <SectionCard
            icon={AlertTriangle}
            title="ISSUES & FAILURES LOG"
            description="Repeated confusions, weak anchors, and system failures grouped by frequency."
            testId="brain-issues-log"
          >
            <div className="grid gap-3">
              <div
                className="border border-primary/15 bg-black/30 p-3"
                data-testid="brain-confusions"
              >
                <div className="font-arcade text-ui-xs text-primary">
                  REPEATED CONFUSIONS
                </div>
                <div className="mt-3 space-y-2">
                  {(metrics?.recentConfusions || []).length ? (
                    metrics?.recentConfusions.map((item) => (
                      <div
                        key={`${item.course}-${item.text}`}
                        className="border border-primary/10 bg-black/35 px-3 py-2"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="font-terminal text-xs text-white">
                    ...
                    <div className="font-terminal text-xs text-muted-foreground">
                      No repeated confusions captured.
                    </div>
                  )}
                </div>
              </div>
              <div
                className="border border-primary/15 bg-black/30 p-3"
                data-testid="brain-weak-anchors"
              >
                <div className="font-arcade text-ui-xs text-primary">
                  WEAK ANCHORS
                </div>
                <div className="mt-3 space-y-2">
                  {(metrics?.recentWeakAnchors || []).length ? (
                    metrics?.recentWeakAnchors.map((item) => (
                      <div
                        key={`${item.course}-${item.text}`}
                        className="border border-primary/10 bg-black/35 px-3 py-2"
                      >
                    ...
                    <div className="font-terminal text-xs text-muted-foreground">
                      No repeated weak anchors captured.
                    </div>
                  )}
                </div>
              </div>
              <div
                className="border border-primary/15 bg-black/30 p-3"
                data-testid="brain-issue-patterns"
              >
                <div className="font-arcade text-ui-xs text-primary">
                  WORKFLOW / SYSTEM ISSUES
                </div>
                <div className="mt-3 space-y-2">
                  {(metrics?.issuesLog || []).length ? (
                    metrics?.issuesLog.map((item) => (
                      <div
                        key={`${item.course}-${item.issue}`}
                        className="border border-primary/10 bg-black/35 px-3 py-2"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="font-terminal text-xs text-white">
        ...
        <SectionCard
          ...
          description="Display-only connection and evidence surfaces. No editing from Brain."
          testId="brain-integrations"
        >
          <div className="grid gap-3 xl:grid-cols-2">
            <div
              className="border border-primary/15 bg-black/30 p-3"
              data-testid="brain-integration-obsidian"
            >
              <div className="flex items-center justify-between gap-3">
                <div className="font-arcade text-ui-xs text-primary">
                  OBSIDIAN
                ...
                <Badge
                  ...
                  className={cn(
                    ...
                    workspace.obsidianStatus?.connected
              ...
              <div className="mt-3 grid gap-2 font-terminal text-xs text-muted-foreground">
                <div className="border border-primary/10 px-3 py-2">
                  Notes created per session: not yet modeled in Brain API
                </div>
                <div className="border border-primary/10 px-3 py-2">
                  Notes edited: not yet modeled in Brain API
                </div>
                <div className="border border-primary/10 px-3 py-2">
                  Concept links / orphan notes: stub until session-note linkage
                  is exposed
                </div>
              </div>
            </div>
            <div
              className="border border-primary/15 bg-black/30 p-3"
              data-testid="brain-integration-anki"
            >
              <div className="flex items-center justify-between gap-3">
                <div className="font-arcade text-ui-xs text-primary">ANKI</div>
                <Badge
                  variant="outline"
                  className={cn(
                    "rounded-none text-ui-2xs",
                    workspace.ankiStatus?.connected
                      ? "border-emerald-500/40 text-emerald-300"
                      : "border-amber-500/40 text-amber-200",
                  )}
                >
                  {workspace.ankiStatus?.connected ? "CONNECTED" : "OFFLINE"}
                </Badge>
              </div>
              <div className="mt-3 grid gap-2 font-terminal text-xs text-muted-foreground">
                <div className="border border-primary/10 px-3 py-2">
                  Cards created from WRAP sessions: {metrics?.totalCards || 0}
                </div>
                <div className="border border-primary/10 px-3 py-2">
                  Pending drafts waiting on review:{" "}
                ...
                <div className="border border-primary/10 px-3 py-2">
                  ...
                  {ankiDueUnavailable
                    ? "unavailable"
                    : `${ankiDue?.dueCount || 0} due card(s)`}
                </div>
                <div className="border border-primary/10 px-3 py-2">
                  Lapses: not yet modeled in Brain API
                </div>
              </div>
            </div>
          </div>
        ...
        <SectionCard
          icon={Sparkles}
          title="LLM ANNOTATION WINDOW"
          description="Paste WRAP notes or upload a session file for organization, tags, and summaries. This surface never writes to stored Brain data."
          testId="brain-llm-organizer"
        >
          <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
            <div className="space-y-4">
              ...
              <div className="font-terminal text-ui-xs text-muted-foreground">
                {uploadedFileName
                  ...
                  : "Accepted formats: .txt, .md, .json"}
              </div>
              <div className="border border-primary/15 bg-black/30 p-3 font-terminal text-xs text-muted-foreground">
                Annotation only. No save, no sync, no card creation, no workflow
                changes.
              </div>
              <Button
                type="button"
                className="w-full rounded-none font-arcade text-xs"
                disabled={!rawNotes.trim() || organizeMutation.isPending}
                onClick={() =>
                  organizeMutation.mutate({
                    rawNotes,
                    course: courseHint.trim() || undefined,
                  })
                }
              >
            ...
            <div className="space-y-3">
              {organizeMutation.isError ? (
                <div className="border border-destructive/35 bg-destructive/10 p-3 font-terminal text-xs text-destructive">
                  {organizeMutation.error instanceof Error
                    ? organizeMutation.error.message
                    : "Could not organize this session input."}
                </div>
              ) : null}
              {organizeMutation.data?.organized ? (
                <OrganizerPanel data={organizeMutation.data} />
              ) : (
                <div className="flex min-h-[260px] items-center justify-center border border-dashed border-primary/20 bg-black/20 p-6 text-center">
                  <div className="space-y-2">
                    <div className="font-arcade text-xs text-primary">
                      ANNOTATION OUTPUT
                    </div>
                    <div className="font-terminal text-xs text-muted-foreground">
                      Run the organizer to preview tags, structure, and markdown
                      without mutating stored Brain data.
                    </div>
                  </div>
                </div>
              )}
    ...
    <div className="space-y-3">
      <div className="border border-primary/15 bg-black/30 p-3">
        ...
        <div className="mt-2 font-terminal text-sm text-white">
          {data.organized?.title}
        </div>
        {data.destination?.recommended_label ? (
          <div className="mt-2 font-terminal text-ui-xs text-muted-foreground">
            Suggested destination: {data.destination.recommended_label}
          </div>
        ) : null}
      </div>
      <div className="grid gap-3 xl:grid-cols-[0.85fr_1.15fr]">
        <div className="space-y-3">
          <div className="border border-primary/15 bg-black/30 p-3">
            <div className="font-arcade text-ui-xs text-primary">CHECKLIST</div>
            <div className="mt-3 space-y-2">
              {(data.organized?.checklist || []).length ? (
                data.organized?.checklist.map((item) => (
                  <div
                    key={item}
                    className="border border-primary/10 bg-black/35 px-3 py-2 font-terminal text-xs text-white"
                  >
                    {item}
                  </div>
                ))
          ...
          <div className="border border-primary/15 bg-black/30 p-3">
            ...
            <div className="mt-3 space-y-2">
              {(data.organized?.suggested_links || []).length ? (
                data.organized?.suggested_links.map((item) => (
                  <div
                    key={item}
                    className="border border-primary/10 bg-black/35 px-3 py-2 font-terminal text-xs text-white"
                  >
        ...
        <div className="border border-primary/15 bg-black/30 p-3">
          ...
          <pre className="mt-3 max-h-[360px] overflow-auto whitespace-pre-wrap font-terminal text-xs text-muted-foreground">
            {data.organized?.markdown}
          </pre>
        </div>
      </div>
    </div>
