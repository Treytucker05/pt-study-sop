...
export function MemoryPanel({
  ...
  const historyEntries = status.history.slice(status.latestCapsule ? 1 : 0, 5);
          ...
          <div>
            ...
            {status.latestCapsule ? (
              <>
                ...
                <div className="mt-3">
                  {activeCapsuleId === status.latestCapsule.id ? (
                    <span className="text-[10px] uppercase tracking-[0.18em] text-primary/78">
                      Active continuation capsule
                    </span>
                  ) : (
                    <button
                      type="button"
                      className="rounded-full border border-primary/20 px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-primary/84 transition-colors hover:border-primary/35 hover:text-primary"
                      onClick={() => onActivateCapsule?.(status.latestCapsule!.id)}
                    >
                      Resume from {status.latestCapsule.versionLabel}
                    </button>
                  )}
                </div>
              </>
                    ...
                    <div className="mt-3">
                      {activeCapsuleId === entry.id ? (
                        ...
                        <button
                          type="button"
                          className="rounded-full border border-primary/20 px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-primary/84 transition-colors hover:border-primary/35 hover:text-primary"
                          onClick={() => onActivateCapsule?.(entry.id)}
                        >
                          Resume from {entry.versionLabel}
                        </button>
                      )}
                    </div>
                  </div>
