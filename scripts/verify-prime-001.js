...
const FORBIDDEN_METHOD_IDS = ["M-PRE-002", "M-PRE-005"];

const page = await browser.getPage("verify-prime-001");
const consoleErrors = [];
...
try {
  ...
  if (!courseChoice?.courseId) {
    throw new Error("No course with materials available for PRIME-001 verification");
  }
  ...
  const summary = await page.evaluate(() => {
    ...
    return cards.map((card) => ({
      ...
      description:
        card
          .querySelector('[data-testid="priming-method-card-description"]')
          ?.textContent?.trim() || "",
      energy:
        card
          .querySelector('[data-testid="priming-method-card-energy"]')
          ?.textContent?.trim() || "",
      duration:
        card
          .querySelector('[data-testid="priming-method-card-duration"]')
          ?.textContent?.trim() || "",
    }));
  });

  check("Exactly 8 priming methods are listed", summary.length === 8, JSON.stringify(summary));
  check(
    "Only the pure-priming method IDs are listed",
    JSON.stringify(summary.map((item) => item.methodId)) === JSON.stringify(REQUIRED_METHOD_IDS),
    JSON.stringify(summary.map((item) => item.methodId)),
  );
  check(
    "M-PRE-002 is absent",
    !summary.some((item) => item.methodId === "M-PRE-002"),
  );
  check(
    "M-PRE-005 is absent",
    !summary.some((item) => item.methodId === "M-PRE-005"),
  );
  check(
    "Each method card still shows description, energy cost, and duration",
    summary.every(
      (item) =>
        item.description.length > 0 &&
        /energy/i.test(item.energy) &&
        /min/i.test(item.duration),
    ),
    JSON.stringify(summary),
  );
  check(
    "Forbidden method IDs are not present anywhere in the picker summary",
    FORBIDDEN_METHOD_IDS.every(
      (methodId) => !summary.some((item) => item.methodId === methodId),
    ),
  );
