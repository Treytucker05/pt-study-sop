...
## Concise plan mode (explicitly requested)
If the user explicitly asks for a short or fast plan, use this mode:
- Keep discovery read-only and light.
- Ask at most 1-2 blocking questions; if not blocked, proceed with clear assumptions.
- Use the concise template below.
