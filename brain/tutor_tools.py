...
Tools:
  ...
  2. apply_obsidian_write_preview — apply a pending Obsidian write preview
  3. list_obsidian_paths        — list files/folders in the Obsidian vault
  4. read_obsidian_note         — read markdown note content from the vault
  5. search_obsidian_notes      — search notes in the vault by query
  ...
  9. save_learning_objectives   — persist approved LOs to DB and rebuild Map of Contents
...
APPLY_OBSIDIAN_WRITE_PREVIEW_SCHEMA: dict[str, Any] = {
    ...
    "description": (
        ...
        "Use preview_id from save_to_obsidian output, or omit it to apply the "
        "latest pending preview for the current session."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
def execute_apply_obsidian_write_preview(
    ...
    *,
    ...
    """Apply a pending Obsidian write preview for the current session."""
    try:
...
def execute_list_obsidian_paths(arguments: dict[str, Any]) -> dict[str, Any]:
    """List files/folders from a vault-relative folder path."""
    ...
    try:
        ...
        files.sort(key=lambda value: (0 if value.endswith("/") else 1, value.lower()))
        return {
            "success": True,
