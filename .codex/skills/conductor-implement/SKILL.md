...
This will update canonical docs (if applicable) and ensure README links to them (if README isn't canonical):
- conductor/product.md (if new features added)
- conductor/tech-stack.md (if new dependencies added)
- README.md (as an index; only add/refresh links unless README is canonical)
- Run any doc drift check scripts (if the repo has them)
