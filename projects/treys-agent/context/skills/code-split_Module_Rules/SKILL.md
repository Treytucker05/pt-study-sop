...
## Minimal questions to ask (donâ€™t skip)
...
2) Propose the **nested** `AGENTS.md` placement(s) and get confirmation.
   - If the only detected module is repo root, suggest a subdir (e.g., `src/`, `apps/<name>/`, `packages/<name>/`) or ask the user where the code â€œstartsâ€.
