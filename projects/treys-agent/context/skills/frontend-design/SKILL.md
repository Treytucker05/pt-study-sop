...
## UI/UX Pro Max research workflow (optional)

Use when you need rapid guidance on style, color, typography, layout, or stack best practices.

Search script (auto-domain or explicit domain):
```bash
...
Notes:
- Data lives in `~/.codex/skills/ui-ux-pro-max/data`.
- If the user did not specify a stack, default to html-tailwind.

## Artifact builder for complex UI artifacts

Use when the request needs multi-component React, state, routing, or shadcn/ui.

Resources: `~/.codex/skills/artifacts-builder/scripts`

Steps:
1) Init: `bash ~/.codex/skills/artifacts-builder/scripts/init-artifact.sh <project-name>`
2) Build your artifact in the generated project.
3) Bundle: `bash ~/.codex/skills/artifacts-builder/scripts/bundle-artifact.sh`
4) Share the resulting `bundle.html`.
