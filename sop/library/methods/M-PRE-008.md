...
Produce: StructuralSpine, Objective linkage map, UnknownNodeList, PriorityNodes, and FullTopicMap.
...
After the compact spine, also produce a FullTopicMap: an expanded objective-organized roadmap covering all high-yield branches plus supporting examples, each node labeled core or example, examples nested under the node they illustrate, no new objectives from supplemental sources.
Preset behavior:
