...
class TestRuntimeMethodsBundle:
    ...
    def test_runtime_matches_golden(self) -> None:
        sys.path.insert(0, str(TOOLS_DIR))
        from build_runtime_bundle import build_methods_from_yaml_direct

        generated = build_methods_from_yaml_direct()
        golden_path = GOLDEN_DIR / "06_METHODS.golden.md"
        assert golden_path.exists(), "Golden file missing — run --update-golden"
        golden_full = golden_path.read_text(encoding="utf-8")
        assert len(golden_full.strip()) > 100, "Golden file is unexpectedly small"
        # Golden includes the render_header(); strip it to compare body only.
        # Header ends at first "---\n\n" separator.
        sep = "---\n\n"
        idx = golden_full.find(sep)
        golden_body = golden_full[idx + len(sep):] if idx != -1 else golden_full
        assert generated.strip() == golden_body.strip(), (
            "Generated 06_METHODS.md (direct from YAML) does not match golden. "
            "If intentional, run: python sop/tools/build_runtime_bundle.py --update-golden"
        )
