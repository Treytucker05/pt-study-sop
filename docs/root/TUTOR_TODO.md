...
- [x] OPS-NEW-COMPUTER-INSTALL-2026-05-10. Productize new-computer install and macOS launch path.
  - Scope landed:
    ...
    - Follow-up 2026-05-10: removed the Windows-only default PT School path from the Library sync input, added `PT_GCAL_REDIRECT_URI`, added Google Calendar client dependencies to `brain/requirements.txt`, and fixed the local-data bundle scripts to include `brain/data/gcal_token.json` plus `brain/data/vault_courses.yaml`.
  - Assignee: @codex-cli
...
- [x] STUDIO-CANVAS-UX-001. Studio canvas UX hardening pass: Source Shelf Fit, Tidy Up regression, missing Center button, hero meta row wasted space, one-direction canvas pan.
  - Scope landed (all changes in `dashboard_rebuild/client/src/`):
    - `components/ui/WorkspacePanel.tsx` — `Fit content` measurement now temporarily disables Tailwind `truncate` / `overflow-hidden` on descendants via a scoped `<style>` + `data-workspace-panel-fit-measuring` attribute so `scrollWidth` / `getBoundingClientRect` reflect the natural unclipped text width. Fixes Source Shelf Fit resizing the panel to the already-truncated filename width. Style element is installed once and reused; attribute is cleaned up in the `finally` block even on measurement errors.
    - `components/studio/StudioShell.tsx`:
      - Added `centerOpenPanels()` callback and a dedicated "Center" toolbar button (Crosshair icon) next to Tidy Up. Center pans the viewport to the centroid of all open panels at the current zoom, using `buildStudioShellViewportCenter`. Distinct from "Fit to View" which rescales.
      - Tidy Up now resets each panel to its per-panel `PRESET_LAYOUT_DEFAULTS[panel].defaultSize` before tiling, so previously Fit-expanded or manually resized panels don't leave large gaps in the re-tiled grid. Then applies `focusOpenPanels` via `shouldFocusLayout`.
      - `TransformWrapper` now sets `limitToBounds={false}`, `centerOnInit={false}`, `centerZoomedOut={false}`. `limitToBounds` defaulted to `true`, which clamped `positionX >= 0` and made the canvas pan one direction only ("scrolls right but not left; left side cut off"). Disabling it lets the custom pointer-drag handler freely reveal either side of the workspace.
    - `index.css` — `.page-shell__meta` is now a single CSS grid with `grid-template-columns: repeat(auto-fit, minmax(10rem, 1fr))`; `.page-shell__stat-grid` and `.page-shell__actions` are `display: contents` so stat tiles and hero action buttons flatten into the same grid. Result: Tutor hero's `NEW SESSION` / `REFRESH` fill the empty cells next to `MATERIALS` instead of stacking on a new row below the stats. Action children get `width: 100%; min-width: 0;` so they stretch cleanly into their grid cell.
    - Earlier companion work in this same UX pass (already on `main`): `components/studio/SourceShelf.tsx` display of basenames with full-path tooltips, `lib/pathDisplay.ts` shared utility, `scripts/audit_material_titles.py` backfill of `rag_docs.title`, `StudioDocumentDock.tsx` basename display, `studioPacketSections.ts` basename in Prime Packet source details, `WorkspacePanel.tsx` icon-only title-bar controls, floating bottom-right nav cluster (zoom slider, Fit, 100%), `Ctrl+wheel` native non-passive zoom listener, click-to-front + Windows popover, collision-avoidance pusher `resolveStudioShellPanelCollisions`, Start Session renames across `tutor.tsx` / `TutorShell.tsx` / `TutorStudioHome.tsx` and the workflow-level END SESSION semantics.
  - Validation:
    - `dashboard_rebuild> npm run build` — green (bundles rebuilt into `brain/static/dist/`).
    - Browser verification previously performed for the Start-Session rename, workflow-level END SESSION, and file-tree basename display; `STUDIO-CANVAS-UX-001` scope (Fit / Tidy / Center / pan bounds / hero meta grid) is pending a manual/browser pass after the next dashboard restart.
    - Vitest deferred: `vitest` has been hanging/locking the workstation on this machine; tests were authored earlier in the pass for basename display (`SourceShelf.test.tsx`) and workflow-level END SESSION (`tutor.test.tsx`) but run on next available stable vitest session.
  - Recommended next steps (pick up here):
    1. Manual/browser verify after `Start_Dashboard.bat` restart: Source Shelf `Fit` now sizes wide enough to show full filenames; Tidy Up resets sizes + tiles cleanly; Center pans without zoom change; canvas drag works both directions; Tutor hero shows NEW SESSION / REFRESH on the stat row next to MATERIALS.
    2. Add Vitest coverage: (a) `measureFitContentSize` returns a width that is not less than the natural text width when children carry `truncate`; (b) Tidy Up normalizes resized panels back to `PRESET_LAYOUT_DEFAULTS[panel].defaultSize`; (c) `PageScaffold` renders stats + actions as siblings in a single grid parent.
    3. Resume TUTOR-AUDIT-P2-001 — it remains the next major backlog item.
    4. Confirm the previously-flagged unrelated red test `client/src/components/__tests__/TutorShell.test.tsx "stale Tutor session id"` (bundle.ts:225 `input.artifacts` undefined) is still outstanding; if yes, queue a dedicated TutorShell-hardening task.
  - Assignee: @claude-cursor
  - Completed: 2026-04-22
...
- [ ] HUD-239. Execute the floating-panel Studio v2 cutover from `docs/design/STUDIO_LAYOUT_SPEC_v2.md` using the slice-by-slice TDD runbook.
  ...
  - Progress:
    ...
    - Slice 3.5 in progress (2026-03-27): rebuild `SourceShelf.tsx` from the flat Current Run / Library / Vault lists into one searchable, collapsible course tree with checkbox-driven run membership, folder-level bulk selection, inline upload-to-library, and leaf-level open actions that keep Document Dock / Prime Packet / Priming loaded-material state in sync through the existing Studio run arrays.
