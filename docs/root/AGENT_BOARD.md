...
|---|---|---|---|---|---|---|
| `codex-orchestrator-prime-trace` | `INV-TUTOR-TRACE-001` integrate Prime -> Tutor -> post-Tutor flow map | read-only: `README.md`, `docs/root/PROJECT_ARCHITECTURE.md`, `docs/root/TUTOR_TODO.md`, `docs/root/AGENT_BOARD.md` | `done` | `none` | integrated trace returned in chat with concrete file anchors across workflow, session, turn, Studio, and publish paths | no further handoff |
| `codex-prime-shard` | `INV-TUTOR-TRACE-001` find where Priming/Prime starts, what artifacts it creates, and where they are stored | read-only: `dashboard_rebuild/client/src/components/TutorWorkflowPrimingPanel.tsx`, `dashboard_rebuild/client/src/lib/studioPacketSections.ts`, `brain/dashboard/api_tutor_workflows.py`, `brain/dashboard/api_tutor_studio.py`, related types/tests | `done` | `none` | local trace anchored in `useTutorWorkflow.ts`, `TutorWorkflowPrimingPanel.tsx`, `studioPacketSections.ts`, and `api_tutor_workflows.py` | no further handoff |
| `codex-handoff-shard` | `INV-TUTOR-TRACE-001` trace how Priming reaches Tutor session creation and live turn execution | read-only: `dashboard_rebuild/client/src/pages/tutor.tsx`, `brain/dashboard/api_tutor_sessions.py`, `brain/dashboard/api_tutor_turns.py`, related tests | `done` | `none` | local trace anchored in `useTutorWorkflow.ts`, `useTutorSession.ts`, `api_tutor_sessions.py`, and `api_tutor_turns.py` | no further handoff |
| `codex-output-shard` | `INV-TUTOR-TRACE-001` trace what happens after Tutor and where artifacts/results go | read-only: `dashboard_rebuild/client/src/components/TutorWorkflowPolishStudio.tsx`, `dashboard_rebuild/client/src/components/TutorWorkflowFinalSync.tsx`, `brain/dashboard/api_tutor_artifacts.py`, `brain/dashboard/api_tutor_sessions.py`, `brain/dashboard/api_tutor_workflows.py`, related types/tests | `done` | `none` | storage/publish map returned from shard and merged into final trace | no further handoff |
| `codex-hud248-frontend` | `HUD-248` add Studio zoom slider and per-panel size presets on `/tutor` | write: `dashboard_rebuild/client/src/components/studio/StudioShell.tsx`, `dashboard_rebuild/client/src/components/studio/__tests__/StudioShell.test.tsx`, `dashboard_rebuild/client/src/components/ui/WorkspacePanel.tsx`, `dashboard_rebuild/client/src/components/__tests__/WorkspacePanel.test.tsx` | `done` | `none` | focused HUD-248 diff landed with green validation: `cd dashboard_rebuild && npx vitest run client/src/components/__tests__/WorkspacePanel.test.tsx client/src/components/studio/__tests__/StudioShell.test.tsx`, `cd dashboard_rebuild && npm run build` | no further handoff; orchestrator can close the Studio slice |
| `codex-search-sop` | `MTH-SEARCH-001` scan canonical SOP methods for mind-map, ASCII graph, and priming-style patterns | read-only: `sop/library/`, `sop/templates/`, method/prompt docs | `done` | `none` | canonical hits confirmed in `M-PRE-004`, `M-PRE-005`, `M-PRE-009`, `M-PRE-010`, `M-ENC-015`, plus `15-method-library.md` / `categories/PRIME.md` | orchestrator to report canonical method shortlist first |
