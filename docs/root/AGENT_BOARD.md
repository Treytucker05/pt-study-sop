...
| `codex-implement` | `TLR-001` open the Tutor launch/shell realignment track and make it the active planning surface | `conductor/tracks/tutor-launch-shell-realignment_20260313/`, `conductor/tracks.md`, `docs/root/TUTOR_TODO.md`, `docs/root/AGENT_BOARD.md`, `conductor/tracks/GENERAL/log.md`, plus the stale/synced Tutor planning docs named in the new track plan | `in_progress` | none | new durable track opened with spec/plan/findings/review/validation artifacts; workboard/registry sync in progress | next handoff should sync stale Tutor planning artifacts and mark pre-shell docs historical before touching frontend code |
