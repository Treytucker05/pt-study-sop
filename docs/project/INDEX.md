# Project Hub

Single canonical entrypoint for planning and execution. Start here.

## Canonical docs
- [PRD](../prd/PT_STUDY_OS_PRD_v1.0.md)
- [Tutor SOP Explorer spec](../dashboard/TUTOR_PAGE_SOP_EXPLORER_v1.0.md)
- [SOP manifest](../../sop/sop_index.v1.json)

## Current milestone
- [CURRENT_MILESTONE.md](CURRENT_MILESTONE.md)
- Brain ingestion MVP PRs:
  - [ ] PR-01
  - [ ] PR-02
  - [ ] PR-03
  - [ ] PR-04
  - [ ] PR-05

## How to run
- [Start_Dashboard.bat](../../Start_Dashboard.bat)
- [scripts/test_brain_ingest.sh](../../scripts/test_brain_ingest.sh)

## Quality gates
- `python scripts/audit_repo_hygiene.py`

## Where things live
- [docs/README.md](../README.md)
- [JANUARY_26_PLAN/INDEX.md](../../JANUARY_26_PLAN/INDEX.md)

## AI workflows
- [AI Review Loop](AI_REVIEW_LOOP.md)
