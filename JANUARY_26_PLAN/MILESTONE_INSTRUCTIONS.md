...
```
C:\pt-study-sop\dashboard_rebuild\
├── schema.ts                      ← Database schema (ADD TABLES HERE)
├── shared\                        ← DOES NOT EXIST - MUST CREATE
│   └── schema.ts                  ← Re-export from ../schema.ts
├── data.db                        ← SQLite database file
├── drizzle.config.ts              ← Drizzle config
├── package.json                   ← npm scripts
├── tsconfig.json                  ← TypeScript config (has @shared/* alias)
├── server\
│   ├── db.ts                      ← Database connection
│   ├── storage.ts                 ← Storage class (ADD CRUD HERE)
│   ├── routes.ts                  ← API routes (ADD ENDPOINTS HERE)
│   └── index.ts                   ← Server entry
└── client\src\
    ├── api.ts                     ← API client (ADD CALLS HERE)
    ├── App.tsx                    ← Router
    ├── pages\
    │   ├── brain.tsx              ← Brain page (ADD TABS HERE)
    │   ├── dashboard.tsx          ← Dashboard (ADD WIDGETS HERE)
    │   └── tutor.tsx              ← Tutor page
    └── components\
...
**Expected output:** Success message, tables created

**Validation:**
```powershell
# Open SQLite and check tables exist
# You can use DB Browser for SQLite or:
...
**File:** `C:\pt-study-sop\dashboard_rebuild\server\storage.ts`

---

## Step 2.1: Update Imports in storage.ts
...
**File:** `C:\pt-study-sop\dashboard_rebuild\server\routes.ts`

---

## Step 3.1: Update Imports in routes.ts
...
- [ ] Schema imports updated in routes.ts
      ...
      {selectedCourseId && (
        <>
          ...
          <div className="border border-green-500/30 rounded p-4">
            <h3 className="text-lg font-semibold mb-2">📅 Schedule Import</h3>
            <button
          ...
          {/* LO Import Section */}
          <div className="border border-green-500/30 rounded p-4">
            <h3 className="text-lg font-semibold mb-2">🎯 Learning Objectives Import</h3>
...
1. ✅ **Milestone 1** - Schema (30 min)
2. ✅ **Milestone 2** - Storage (45 min)
3. ✅ **Milestone 3** - Routes (30 min)
4. ✅ **Milestone 4** - API Client (15 min)
5. ⏸️ **Milestone 5** - Prompts (10 min) - Can use prompts manually
6. ⏸️ **Milestone 6** - UI (Optional tonight)
